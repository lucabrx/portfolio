    ...
    content: (
      <div className="flex flex-col space-y-2 justify-center items-start">
        {frontendTools.map((tool, index) => (
          <div
            className="flex items-center justify-start  max-w-[120px] w-full"
            key={index}
          >
            <div className="mr-4 p-2 aspect-square w-10">
              {<tool.Icon className="h-6 aspect-square" />}
            </div>
    ...
    content: (
      <div className="flex flex-col space-y-2 justify-center items-start">
        {backendTools.map((tool, index) => (
          <div
            className="flex items-center justify-start  max-w-[125px] w-full"
            key={index}
          >
            <div className="mr-4 p-2 aspect-square w-10">
              {<tool.Icon className="h-6 aspect-square" />}
    ...
    content: (
      <div className="flex flex-col space-y-2 justify-center items-start">
        {designTools.map((tool, index) => (
          <div
            className="flex items-center justify-start  max-w-[145px] w-full"
            key={index}
          >
            <div className="mr-4 p-2 aspect-square w-10  ">
              {<tool.Icon className="h-6 aspect-square" />}
      ...
      <div className="pt-7 grid grid-cols-1 lg:grid-cols-2 gap-7">
        ...
        <div
          className={cn(
            ...
            inView ? "animate-fade-in [--animation-delay:600ms]" : "",
          )}
        >
