...
const ContactSection: FC = () => {
  ...
  <div ref={ref}  className='w-full  md:mt-8  '>
    ...
    <h2 className={cn("w-full text-center lg:text-4xl xl:text-5xl text-3xl font-semibold opacity-0 pt-8 pb-4", inView ? "animate-fade-in" : "")}>
    Say Hello 🤝
    </h2>
  <div className='grid grid-cols-1 md:grid-cols-[45%,1fr] container items-stretch md:gap-7 xl:gap-10'>
  
  <Image src="/contact.svg" alt="potrait" width={420} height={420} className={cn('md:block hidden rounded-md drop-shadow-sm opacity-0', inView ? "animate-fade-in [--animation-delay:400ms]" : "")}  />

  <div className={cn("flex flex-col items-center justify-center w-full opacity-0", inView ? "animate-fade-in [--animation-delay:400ms] md:[--animation-delay:600ms]" : "")}>
  <ContactForm />
  </div>
